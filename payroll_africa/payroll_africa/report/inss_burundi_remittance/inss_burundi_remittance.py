...
def get_data(filters):
	...
	for row in data:
		emp = flt(frappe.db.get_value(
			"Salary Detail",
			{"parent": row.salary_slip, "salary_component": "INSS Employee BI", "parentfield": "deductions"},
			"amount",
		))
		empr = flt(frappe.db.get_value(
			"Salary Detail",
			{"parent": row.salary_slip, "salary_component": "INSS Employer BI", "parentfield": "deductions"},
			"amount",
		))
		risk = flt(frappe.db.get_value(
			"Salary Detail",
			{"parent": row.salary_slip, "salary_component": "Work Injury BI", "parentfield": "deductions"},
			"amount",
		))
