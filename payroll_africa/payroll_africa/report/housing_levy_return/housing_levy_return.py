...
def get_data(filters):
	...
	for row in data:
		emp_levy = frappe.db.get_value(
			"Salary Detail",
			{"parent": row.salary_slip, "salary_component": "Housing Levy", "parentfield": "deductions"},
			"amount",
		) or 0
		empr_levy = frappe.db.get_value(
			"Salary Detail",
			{"parent": row.salary_slip, "salary_component": "Employer Housing Levy", "parentfield": "deductions"},
			"amount",
		) or 0
