...
def get_columns():
	return [
		...
		{"fieldname":"nsf_empr", "label":_("NSF Employer"), "fieldtype":"Currency", "width":130},
		{"fieldname":"hrdc", "label":_("HRDC Levy"), "fieldtype":"Currency", "width":130},
	]
...
def get_data(filters):
	...
	amounts = fetch_component_amounts(slip_names, ['NSF Employee', 'NSF Employer', 'HRDC Levy'])

	for row in data:
		slip_amounts = amounts.get(row.salary_slip, {})
		row["nsf_emp"] = slip_amounts.get("NSF Employee", 0)
		row["nsf_empr"] = slip_amounts.get("NSF Employer", 0)
		row["hrdc"] = slip_amounts.get("HRDC Levy", 0)
		del row["salary_slip"]
