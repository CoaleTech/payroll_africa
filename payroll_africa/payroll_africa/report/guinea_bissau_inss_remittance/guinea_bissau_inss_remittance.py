...
def get_data(filters):
	...
	amounts = fetch_component_amounts(slip_names, ['INSS Employee', 'INSS Employer'])

	for row in data:
		slip_amounts = amounts.get(row.salary_slip, {})
		row["inss_emp"] = slip_amounts.get("INSS Employee", 0)
		row["inss_empr"] = slip_amounts.get("INSS Employer", 0)
		del row["salary_slip"]
