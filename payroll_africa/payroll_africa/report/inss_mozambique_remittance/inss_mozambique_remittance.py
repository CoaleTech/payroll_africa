...
def get_columns():
	return [
		...
		{"fieldname": "mz_inss_no", "label": _("INSS No"), "fieldtype": "Data", "width": 130},
		...
		{"fieldname": "inss_employer", "label": _("Employer (4%)"), "fieldtype": "Currency", "width": 130},
		{"fieldname": "total_inss", "label": _("Total (7%)"), "fieldtype": "Currency", "width": 120},
	]
...
def get_data(filters):
	...
	data = frappe.db.sql(
		...
		SELECT
			...
			e.mz_inss_no, ss.name as salary_slip
	...
	for row in data:
		emp = flt(frappe.db.get_value(
			"Salary Detail",
			{"parent": row.salary_slip, "salary_component": "INSS Employee MZ", "parentfield": "deductions"},
			"amount",
		))
		empr = flt(frappe.db.get_value(
			"Salary Detail",
			{"parent": row.salary_slip, "salary_component": "INSS Employer MZ", "parentfield": "deductions"},
			"amount",
		))
