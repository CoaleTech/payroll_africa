...
def get_data(filters):
	...
	amounts = fetch_component_amounts(slip_names, ['CNSS Pension Employee', 'CNSS Pension Employer'])

	for row in data:
		slip_amounts = amounts.get(row.salary_slip, {})
		row["cnss_emp"] = slip_amounts.get("CNSS Pension Employee", 0)
		row["cnss_empr"] = slip_amounts.get("CNSS Pension Employer", 0)
		del row["salary_slip"]
